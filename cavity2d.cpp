...
void getResults( SuperLattice2D<T, DESCRIPTOR>& sLattice,
  ...
  if ( iT == converter.getLatticeTime( maxPhysT ) || converged) { // || iT == 0) { //output iT==0
    // Gives access to velocity information on lattice
    //SuperLatticePhysVelocity2D<T, DESCRIPTOR> velocityField( sLattice, converter ); // COMMENTED
    SuperLatticePhysPressure2D<T,DESCRIPTOR> pressure( sLattice, converter );
    // Interpolation functor with velocityField information
    // AnalyticalFfromSuperF2D<T> interpolation( velocityField, true, 1 );
    AnalyticalFfromSuperF2D<T> interpolation( pressure, true, 1 ); //functor pressure
   ...
   // Vector<T,17> comparison = vel_ghia_RE100;

   for ( int nY = 0; nY <= 100; ++nY ) { //nY standard 17
      // 17 data points evenly distributed between 0 and 1 (height)
      //T postition[2] = {0.5, y_coord[nY]/128.0};
      , 
      T velocity[2] = {T(), T()}; //could be changed because Pressure is 1-dimensional
      // Interpolate velocityField at "position" and save it in "velocity"
      interpolation( velocity, postition ); //Information im interpolation object, velocity oder pressure name ist egal
      // Get Console Output after interpolation
      //////////////////////////////////////////
      // Terminal Output
      //////////////////////////////////////////
      clout << "position: " << postition[1]
            << "; velocity: " << velocity[0]
            << std::endl;
