...
// Cuboid Parameter
////////////////////////////////////////
const T xlength = 0.1; //NEW
const T ylength = 0.1; //NEW
const T theta = 0;

////////////////////////////////////////
// 2nd Circle Parameter
////////////////////////////////////////
const T centerCylinderX1 = 0.2+.5;
const T centerCylinderY1 = 0.2+.5*L/2.;
const T radiusCylinder1 = 0.05;

const T radiusCylinder = 0.1; //COMMENTED Standard number 0.05
                      ...
                      SuperGeometry2D<T>& superGeometry )
{
  ...
  Vector<T,2> origin;
  //IndicatorCircle2D<T> circle( center, radiusCylinder ); //COMMENTED
  IndicatorCuboid2D<T> cube(xlength, ylength, center, theta); //NEW
  ...
  Vector<T,2> center1( centerCylinderX1,centerCylinderY1 );

  IndicatorCircle2D<T> circle1( center1, radiusCylinder1 );

  // superGeometry.rename( 0,2 );
  
  // superGeometry.rename( 2,1,1,1 );  // Set material number for inflow
  extend[0] = 2.*L;
  origin[0] = -L;
  ...
  //superGeometry.rename( 1,5,circle ); //COMMENTED THAT'S FOR THE CIRCLE IF NOT COMMENTED PROBLEM WITH LINE 129

  ////////////////////////////////////////
  // Rename to material number
  ////////////////////////////////////////
  superGeometry.rename( 1,5,cube );
  superGeometry.rename( 1,6,circle1 );
   
  // superGeometry.rename( 1,5,circle1 ); //NEW
                     ...
                     SuperGeometry2D<T>& superGeometry )
{
  ...
  sBoundaryCondition.addVelocityBoundary( superGeometry, 3, omega ); //STANDARD VELOCITY BOUNDARY CONDITION
  //sBoundaryCondition.addPressureBoundary( superGeometry, 3, omega ); //NEWNEWNEW
  ...
  offBc.addZeroVelocityBoundary( superGeometry, 5, circle1 ); //BACK TO CIRCLE IF CASE CHANGE
  ...
  sLattice.defineDynamics( superGeometry, 6, &instances::getNoDynamics<T,DESCRIPTOR>() );
  offBc.addZeroVelocityBoundary( superGeometry, 6, cube );

  // Initial conditions
  AnalyticalConst2D<T,T> rhoF( 1 );
  std::vector<T> velocity( 2,T( 0 ) );
  AnalyticalConst2D<T,T> uF( velocity );
  //AnalyticalConst2D<T,T> rho(converter.getLatticeDensityFromPhysPressure(.1*frac[0])); // ADDED
  ...
  // sLattice.defineRho( superGeometry, 3, rho); // ADDED
                 ...
                 SuperGeometry2D<T>& superGeometry, Timer<T>& timer )
{
 ...
 //const int vtkIter  = converter.getLatticeTime( .1 ); //3 mal mehr ausgaben als mit 0.3
